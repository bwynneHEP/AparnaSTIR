...
echo "forward_project my_line_integrals ${emission_image}  ${template_sino} forward_projector_proj_matrix_ray_tracing.par"
forward_project my_line_integrals ${emission_image}  ${template_sino} forward_projector_proj_matrix_ray_tracing.par > my_create_line_integrals.log 2>&1
if [ $? -ne 0 ]; then 
  echo "ERROR running forward_project. Check my_create_line_integrals.log"; exit 1; 
fi

echo "===  create ACFs"
calculate_attenuation_coefficients --ACF my_acfs.hs ${atten_image} ${template_sino} forward_projector_proj_matrix_ray_tracing.par > my_create_acfs.log 2>&1
if [ $? -ne 0 ]; then 
